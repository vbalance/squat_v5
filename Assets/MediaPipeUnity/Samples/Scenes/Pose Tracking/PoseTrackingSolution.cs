...
namespace Mediapipe.Unity.PoseTracking
{
  public class PoseTrackingSolution : ImageSourceSolution<PoseTrackingGraph>
  {
    ...
    protected override IEnumerator WaitForNextValue()
    {
      ...
      {
        //manager.SetHipPos(poseLandmarks.Landmark[0].Y, "left");
        manager.SetHipPos(poseLandmarks.Landmark[23].Y, "left");
        manager.SetHipPos(poseLandmarks.Landmark[24].Y, "right");
      }
    }
    ...
    private void OnPoseLandmarksOutput(object stream, OutputEventArgs<NormalizedLandmarkList> eventArgs)
    {
      ...
      //manager.SetHipPos(eventArgs.value.Landmark[0].Y, "left");
      manager.SetHipPos(eventArgs.value.Landmark[23].Y, "left");
      manager.SetHipPos(eventArgs.value.Landmark[24].Y, "right");
    }
